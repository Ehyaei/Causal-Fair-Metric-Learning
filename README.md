...
Performance Comparison. This figures illustrate the outcomes of the experiment detailed in Table~\ref{tab:metric_5} with different radius size. Error bars are included to represent the variability. Two performance metrics, Matthews correlation coefficient (\textbf{MCC} - higher values indicate superior performance) and false-positive rate (\textbf{FP} - lower values indicate better performance), are considered for the analysis.
